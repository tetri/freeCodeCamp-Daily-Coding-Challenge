...
test("convertListItem 1", () => {
  expect(convertListItem("1. My item")).toStrictEqual("<li>My item</li>");
});

test("convertListItem 2", () => {
  expect(convertListItem(" 1.  Another item")).toStrictEqual("<li>Another item</li>");
});

test("convertListItem 3", () => {
  expect(convertListItem("1 . invalid item")).toStrictEqual("Invalid format");
});

test("convertListItem 4", () => {
  expect(convertListItem("2. list item text")).toStrictEqual("<li>list item text</li>");
});

test("convertListItem 5", () => {
  expect(convertListItem(". invalid again")).toStrictEqual("Invalid format");
});

test("convertListItem 6", () => {
  expect(convertListItem("A. last invalid")).toStrictEqual("Invalid format");
});
